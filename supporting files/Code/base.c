...
void base(){
	...
	macAddressStorage =  malloc(WIDTH * HEIGHT * 17 * sizeof(unsigned char));
	ipAddressStorage =  malloc(WIDTH * HEIGHT * 15 * sizeof(unsigned char));
...
void listenToEvents(){
 ...
 void initializeBaseStation(){
	...
	while (count < (WIDTH * HEIGHT)){
		...
		MPI_Unpack(packbuf, packsize, &position, macAddressStorage + incoming_rank*sizeof(char)*17, 17, MPI_UNSIGNED_CHAR, MPI_COMM_WORLD);
		MPI_Unpack(packbuf, packsize, &position, ipAddressStorage + incoming_rank*sizeof(char)*15, 15, MPI_UNSIGNED_CHAR, MPI_COMM_WORLD);
