...
void base(){
	...
	macAddressStorage =  malloc(WIDTH * HEIGHT * 20 * sizeof(unsigned char));
	ipAddressStorage =  malloc(WIDTH * HEIGHT * 20 * sizeof(unsigned char));
...
void listenToEvents(){
	...
	while (1){
		...
		fprintf (fp, "%s", "\t\t");
		// fwrite(macAddressStorage + incomingNode*sizeof(char)*20 , 20 , sizeof(char) , fp );
		fprintf (fp, "%s",macAddressStorage + incomingNode*sizeof(unsigned char)*20);
		fprintf (fp, "%s", "\t\t");
		// fwrite(ipAddressStorage + incomingNode*sizeof(unsigned char)*20 , 20 , sizeof(unsigned char) , fp );
		fprintf (fp, "%s", ipAddressStorage + incomingNode*sizeof(char)*20);
		fprintf (fp, "%s", "\n\n");
		int labelFlag = 0;
		...
		for (int i = 0; i < 4; i++){
			if (activatedNodes[i] != -1){
				...
				fprintf (fp, "%s", "\t\t");
				// fwrite(macAddressStorage + adjacentNodes[i]*sizeof(unsigned char)*20 , 20 , sizeof(unsigned char) , fp );
				fprintf (fp, "%s", macAddressStorage + adjacentNodes[i]*sizeof(char)*20);
				fprintf (fp, "%s", "\t\t");
				// fwrite(ipAddressStorage + activatedNodes[i]*sizeof(unsigned char)*20 , 20 , sizeof(unsigned char) , fp );
				fprintf (fp, "%s", ipAddressStorage + adjacentNodes[i]*sizeof(unsigned char)*20);
				fprintf (fp, "%s", "\t\t");
 ...
 void initializeBaseStation(){
	...
	while (count < (WIDTH * HEIGHT)){
		...
		MPI_Unpack(packbuf, packsize, &position, macAddressStorage + incoming_rank*sizeof(unsigned char)*20, 20, MPI_UNSIGNED_CHAR, MPI_COMM_WORLD);
		MPI_Unpack(packbuf, packsize, &position, ipAddressStorage + incoming_rank*sizeof(unsigned char)*20, 20, MPI_UNSIGNED_CHAR, MPI_COMM_WORLD);
